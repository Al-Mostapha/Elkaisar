...
{
    return  $.ajax({
        url: `/jsGame${JsVersion}/json/ItemLang/ar.json`,
...
{
    return  $.ajax({
        url: `/jsGame${JsVersion}/json/equipment/ar.json`,
        success: function (data, textStatus, jqXHR) {
...
ElkaisarCp.Ws.connect = function (){
    ...
    ws =  new WebSocket(`ws://${WS_HOST}:${WS_PORT}?token=${OuthToken}`);
...
$(document).on("click", ".examinPlayer", function (){
    ...
    var win = window.open(`http://web.elkaisar.com/ExaminPlayer.php?AdminToken=${OuthToken}&server=&idPlayer=${idPlayer}`, '_blank');
    win.focus();
});