class AWorldUnit {
  ...
  async getWorldUnitDominator() {
    const xCoord = Elkaisar.Base.validateCount(this.Parm.xCoord);
    const yCoord = Elkaisar.Base.validateCount(this.Parm.yCoord);
    const unitType = Elkaisar.Base.validateOffset(this.Parm.unitType);
    let Name = [{ Name: "" }];
    if (Elkaisar.Lib.LWorldUnit.isRepelCastle(unitType) || Elkaisar.Lib.LWorldUnit.isQueenCity(unitType))
      Name = await Elkaisar.DB.ASelectFrom("guild.name AS Name",
        "guild JOIN world_unit_rank ON world_unit_rank.id_dominant = guild.id_guild",
        "world_unit_rank.x = ? AND world_unit_rank.y = ? ORDER BY id_round DESC LIMIT 1",
        [xCoord, yCoord]);
    if (Elkaisar.Lib.LWorldUnit.isArmyCapital(unitType))
      Name = await Elkaisar.DB.ASelectFrom(
        "player.name AS Name", "player JOIN world_unit_rank ON world_unit_rank.id_dominant = player.id_player",
        "world_unit_rank.x = ? AND world_unit_rank.y = ? ORDER BY id_round DESC LIMIT 1", [xCoord, yCoord]
      );
    if (Name.length > 0)
      return Name[0];
    return { Name: "" }
