class AArmyBatch {
  ...
  #getBlockToSpeedUp(buildingPlace) {
    let blockPlaces = [];
  ...
  async #goAndSpeedUp(Batch) {
    ...
    for (let onePlace of commonBlocks) {
      const BatchesInBuilding = await Elkaisar.DB.ASelectFrom("*", "build_army", 
      "id_city = ? AND id_player = ? AND place = ? AND acce = 0", [Batch["id_city"], this.idPlayer, onePlace]);
  ...
  async #speedUpOnePlace(Batches, speedUpFactor) {
    ...
    for (let index = 0; index < Batches.length; index++) {
      if (Batches[index]["acce"] == 1) {
        ...
        newDuration = (Batches[index]["time_end"] - 
        Math.max(Batches[index]["time_start"], now)) - ((Batches[index]["time_end"] - Math.max(Batches[index]["time_start"], now)) * speedUpFactor);
        await Elkaisar.DB.AUpdate("time_start = ?, time_end = ?, acce = 1",  "build_army", "id = ?", [timeStart, timeStart + newDuration, Batches[index]["id"]]);
