class LSaveState {
  ...
  static async foodOutState(idPlayer, idCity, callBack) {
    
    const Heros = await Elkaisar.DB.ASelectFrom("hero_army.*",
      "hero_army JOIN hero ON hero.id_hero = hero_army.id_hero", "hero.id_city = ?", [idCity]);
    let heroFood = 0;
    for (let iii in Heros) {
      var one = Heros[iii];
    ...
    const Found = await Elkaisar.DB.AExist("city_colonize", "id_city_colonized = ?", [idCity]);
    ...
    await Elkaisar.DB.AUpdate(`food_out = LEAST( (army_a*4 + army_b*18 + army_c*36 + army_d*5 + army_e*20 + army_f*150 + ?) 
    * ( 1 - (SELECT  supplying FROM  player_edu WHERE id_player = ?)*3/100 ),  food_in)`,
      "city", "id_city = ?", [heroFood, idPlayer, idCity]);
    if (callBack)
      callBack();
