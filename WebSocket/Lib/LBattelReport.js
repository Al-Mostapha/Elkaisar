class LBattelReport {
  ...
  addPlayer(Player) {
    if (Player["idPlayer"] <= 0)
      return;
    Elkaisar.DB.AInsert(`id_player= ${Player.idPlayer}, id_report= ${this.idReport} , side = ${Player.side}, honor = ${Player.Honor}, time_stamp = ${Date.now() / 1000}`, "report_player", []);
  }
  addHero(Hero) {
    var Query = `id_hero   = ?, id_player = ?, id_report = ?, 
      ...
      side      = ?, ord       = ?, xp       = ?`;
    Elkaisar.DB.AInsert(Query, "report_hero", [
  ...
  addPrize(Player) {
    ...
    if (Sum <= 0)
      return;
    Elkaisar.DB.AInsert(`id_report = ${this.idReport}, id_player = ${Player.idPlayer}, 
