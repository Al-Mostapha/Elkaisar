...
class APlayer
{
    ...
    function getPlayerData(){
        ...
        $Heros = [];
        foreach ($City as $onCity){
            $Heros[$onCity["id_city"]] = selectFromTable("*", "hero", "id_city = :idc", ["idc" => $onCity["id_city"]]);
        }
        
        return [
            "City" => $City,
            "Player" => selectFromTable("*", "player", "id_player = :idp", ["idp" => $idPlayer])[0],
            "Items" => selectFromTable("*", "player_item", "id_player = :idp", ["idp" => $idPlayer]),
