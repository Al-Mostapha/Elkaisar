...
class AWorldUnit {
    ...
    plundePrize() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const Unit = yield Elkaisar.DB.ASelectFrom("*", "world", "x = ? AND y = ?", [xCoord, yCoord]);
            const Domainant = yield Elkaisar.DB.ASelectFrom("*", "world_unit_rank", "x = ? AND y = ? ORDER BY id_round DESC LIMIT 1", [xCoord, yCoord]);
            const playerGuild = yield Elkaisar.DB.ASelectFrom("id_guild, time_join", "guild_member", "id_player = ?", [this.idPlayer]);
            const PrizeTaken = yield Elkaisar.DB.ASelectFrom("*", "world_prize_taken", "x = ? AND y = ? AND id_player = ?", [xCoord, yCoord, this.idPlayer]);
            if (!Domainant.length)
                return { "state": "error_1_0" };
            if (playerGuild.length == 0)
                return { "state": "error_1_1" };
            if (Domainant[0]["id_dominant"] != playerGuild[0]["id_guild"])
                return { "state": "error_1_2" };
            if (PrizeTaken.length > 0)
                return { "state": "error_1_3" };
            if (playerGuild[0]["time_join"] + 3 * 24 * 60 * 60 > Date.now() / 1000)
                return { "state": "error_1_4" };
            var PrizeList = yield Elkaisar.DB.ASelectFrom("*", "world_unit_prize_plunder", "unitType = ? ORDER BY RAND()", [Unit[0]["ut"]]);
            var PrizWin = [];
    ...
    plundeRepleCastlePrize(Unit) {
        return __awaiter(this, void 0, void 0, function* () {
            const Domainant = yield Elkaisar.DB.ASelectFrom("*", "world_unit_rank", "x = ? AND y = ? ORDER BY id_round DESC LIMIT 1", [Unit["x"], Unit["y"]]);
            const playerGuild = yield Elkaisar.DB.ASelectFrom("id_guild, time_join", "guild_member", "id_player = ?", [this.idPlayer]);
            const PrizeTaken = yield Elkaisar.DB.ASelectFrom("*", "world_prize_taken", "x = ? AND y = ? AND id_player = ?", [Unit["x"], Unit["y"], this.idPlayer]);
            if (!Domainant.length)
                return { "state": "error_1_0" };
            if (playerGuild.length == 0)
                return { "state": "error_1_1" };
            ...
            if (playerGuild[0]["time_join"] + 3 * 24 * 60 * 60 > Date.now() / 1000)
                return { "state": "error_1_4" };
            var PrizeList = yield Elkaisar.DB.ASelectFrom("*", "world_unit_prize_plunder", "unitType = ? ORDER BY RAND()", [Unit["ut"]]);
            var PrizWin = [];
            var ii, Luck, onePrize, amount;
            for (ii = 0; ii < PrizeList.length; ii++) {
                onePrize = PrizeList[ii];
