...
class LPrize {
    constructor(Battel) {
        ...
        this.cityAvailResources = {
            food: 0, wood: 0, stone: 0, metal: 0, coin: 0
        };
        this.totalResCap = 0;
        this.seaCityTakenAmount = 0;
        this.SeaCityMaxTakenAmount = 1e9;
        this.Battel = Battel;
    }
    ...
    getCityAvailRes() {
        return __awaiter(this, void 0, void 0, function* () {
            const Res = yield Elkaisar.DB.ASelectFrom("id_city,food,wood,stone,metal,coin, id_player,food_cap,wood_cap, stone_cap,metal_cap,coin_cap", "city", "x = ? AND y = ?", [this.Battel.Battel.x_coord, this.Battel.Battel.y_coord]);
            if (Res[0])
    ...
    takeCityRes() {
        return __awaiter(this, void 0, void 0, function* () {
            yield Elkaisar.Lib.LSaveState.saveCityState(this.cityAvailResources.id_city);
    ...
    cityPrize(Player) {
        ...
        Cities.forEach(function (City, Index) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                yield Elkaisar.Lib.LSaveState.saveCityState(idCity);
                ...
                p_prize["metal"] += metal;
                p_prize["coin"] += coin;
                Elkaisar.DB.Update(`food = food + ${food || 0}, wood = wood + ${wood || 0}, stone = stone + ${stone || 0}, metal = metal + ${metal || 0} , coin = coin + ${coin || 0}`, "city", "id_city = ?", [idCity]);
    ...
    cityItemsPrize(Player) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (var ii in Items) {
                ...
                const PrizeAmount = yield Elkaisar.DB.ASelectFrom("amount", "player_item", "id_player = ? AND id_item = ?", [This.cityAvailResources.id_player, OneItem.prize]);
                if (!PrizeAmount)
                    continue;
                if (!PrizeAmount[0])
                    continue;
                ...
                if (ItemPrize.length >= 10)
                    continue;
                yield Elkaisar.DB.AUpdate(`amount = ${Math.max(0, PrizeAmount[0].amount - amount)}`, "player_item", "id_item = ? AND id_player = ?", [OneItem.prize, This.cityAvailResources.id_player]);
                ItemPrize.push({
                    "prize": OneItem["prize"],
                    "amount_min": amount,
                    "amount_max": amount,
    ...
    givePrize(Player) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (Elkaisar.Lib.LWorldUnit.isCity(Unit["ut"]) && this.Battel.Battel.task == Elkaisar.Config.BATTEL_TASK_CONQUER) {
                const ResourcePrize = yield this.cityPrize(Player);
                const ItemPrize = yield this.cityItemsPrize(Player);
