...
class LAfterFight {
    ...
    updateLvl() {
        ...
        if (Elkaisar.Lib.LWorldUnit.isMonawrat(Unit["ut"]) ||
    ...
    setDominant() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (Elkaisar.Lib.LWorldUnit.isArmyCapital(Unit["ut"]) || Elkaisar.Lib.LWorldUnit.isArena(Unit["ut"])) {
                var idDominant = 0;
                yield Elkaisar.DB.AUpdate("duration = ? - time_stamp", "world_unit_rank", "x = ? AND y = ? ORDER BY id_round DESC LIMIT 1", [now, Unit.x, Unit.y]);
                ...
                if (Elkaisar.Lib.LWorldUnit.isRepelCastle(Unit["ut"]))
                    yield Elkaisar.DB.ADelete("world_unit_rank", "x = ? AND y = ?", [Unit.x, Unit.y]);
    ...
    setCityColonizer() {
        return __awaiter(this, void 0, void 0, function* () {
            var Unit = Elkaisar.World.getUnit(this.Battel.Battel.x_coord, this.Battel.Battel.y_coord);
            var This = this;
            const CityLoy = yield Elkaisar.DB.ASelectFrom("loy, id_city,  player.id_player, city.name AS CityName, player.name AS PlayerName, player.id_guild", "city JOIN player ON player.id_player = city.id_player", "x = ? AND y = ?", [Unit.x, Unit.y]);
            const CityColonizer = yield Elkaisar.DB.ASelectFrom("id_city, player.id_player, player.name AS PlayerName, player.id_guild, player.city_flag", "city JOIN player ON player.id_player = city.id_player", "x = ? AND y = ?", [This.Battel.Battel.x_city, This.Battel.Battel.y_city]);
            const Colonizer = yield Elkaisar.DB.ASelectFrom("*", "city_colonize", "id_city_colonized = ?", [CityLoy[0]["id_city"]]);
            ...
            Elkaisar.DB.Insert(`id_colonizer = ${CityColonizer[0]["id_player"]}, id_colonized = ${CityLoy[0]["id_player"]},
    ...
    setBarrayColonizer() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const BarrCount = yield Elkaisar.DB.ASelectFrom("COUNT(*)as coun", "city_bar", "id_city = ?", [idCity]);
            const PalaceLvl = yield Elkaisar.DB.ASelectFrom("palace", "city_building_lvl", "id_city = ?", [idCity]);
            if (BarrCount[0]["coun"] >= Math.min(PalaceLvl[0]["palace"], 10))
                return;
            yield Elkaisar.DB.ADelete("city_bar", "x_coord = ? AND y_coord = ?", [Unit.x, Unit.y]);
            Elkaisar.DB.Insert("id_player = ?, id_city = ?, x_coord = ?, y_coord = ?", "city_bar", [This.Battel.Battel.id_player, idCity, Unit["x"], Unit["y"]]);
    ...
    setRank() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (Elkaisar.Lib.LWorldUnit.isChallangeFieldPlayer(Unit.ut)) {
                const Res = yield Elkaisar.DB.AQueryExc(`UPDATE arena_player_challange AS a1 JOIN arena_player_challange AS a2 ON( a1.id_player = ? AND a2.id_player = ? )
                            SET a1.rank = a2.rank, a2.rank = a1.rank WHERE a2.rank > a1.rank;`, [This.Battel.Battel["id_player_def"], This.Battel.Battel["id_player"]]);
                Elkaisar.DB.Update("win = win + 1, exp = exp + 3", "arena_player_challange", "id_player = ?", [This.Battel.Battel["id_player"]]);
                Arena = yield Elkaisar.DB.ASelectFrom("exp, lvl, rank", "arena_player_challange", "id_player = ?", [This.Battel.Battel.id_player]);
                ...
                const PlayerTeam = yield Elkaisar.DB.ASelectFrom("id_team", "team_member", "id_player = ?", [This.Battel.Battel["id_player"]]);
                if (!PlayerTeam.length)
                    return;
                var Team = yield Elkaisar.DB.ASelectFrom("name AS TeamName", "team", "id_team = ?", [PlayerTeam[0].id_team]);
                const Res = yield Elkaisar.DB.AQueryExc(`UPDATE arena_team_challange AS a1 JOIN arena_team_challange AS a2 ON( a1.id_team = ? AND a2.id_team = ? )
                            SET a1.rank = a2.rank, a2.rank = a1.rank WHERE a2.rank > a1.rank;`, [This.Battel.Battel["id_player_def"], PlayerTeam[0].id_team]);
                Elkaisar.DB.Update("win = win + 1, exp = exp + 3", "arena_team_challange", "id_team = ?", [PlayerTeam[0].id_team]);
                Arena = yield Elkaisar.DB.ASelectFrom("exp, lvl, rank", "arena_team_challange", "id_team = ?", [PlayerTeam[0].id_team]);
                if (Arena[0]["rank"] == 1 && Res.affectedRows > 0)
                    Elkaisar.Base.broadcast(JSON.stringify({
                ...
                const PlayerGuild = yield Elkaisar.DB.ASelectFrom("id_guild", "guild_member", "id_player = ?", [This.Battel.Battel["id_player"]]);
                if (!PlayerGuild.length)
                    return;
                var Guild = yield Elkaisar.DB.ASelectFrom("guild.name AS GuildName", "guild", "id_guild = ?", [PlayerGuild[0].id_guild]);
                const Res = yield Elkaisar.DB.AQueryExc(`UPDATE arena_guild_challange AS a1 JOIN arena_guild_challange AS a2 ON( a1.id_guild = ? AND a2.id_guild = ? )
                            SET a1.rank = a2.rank, a2.rank = a1.rank WHERE a2.rank > a1.rank;`, [This.Battel.Battel["id_player_def"], PlayerGuild[0].id_guild]);
                Elkaisar.DB.Update("win = win + 1, exp = exp + 3", "arena_guild_challange", "id_guild = ?", [PlayerGuild[0].id_guild]);
                Arena = yield Elkaisar.DB.ASelectFrom("exp, lvl, rank", "arena_guild_challange", "id_guild = ?", [PlayerGuild[0].id_guild]);
                if (Arena[0]["rank"] == 1 && Res.affectedRows > 0)
                    Elkaisar.Base.broadcast(JSON.stringify({
    ...
    afterWin(callBack) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.setColonizer();
            this.setRank();
            yield this.BattelReport.addReport();
            yield this.giveWinnerPrize();
        });
    }
    afterLose(callBack) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    heroBattelBack() {
        ...
        This.Battel.HeroReadyList.forEach(function (OneHero, Index) {
            ...
            if (This.keepGarrisonHero(OneHero))
                return;
            Elkaisar.DB.Insert(`id_hero = ${OneHero.idHero}, x_from  = ${Unit.x}, y_from = ${Unit.y},
    ...
    giveWinnerPrize() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (Elkaisar.Lib.LWorldUnit.isCity(Unit.ut)) {
                if (this.Battel.Battel.task == Elkaisar.Config.BATTEL_TASK_DOMINATE)
                    return;
                yield Prize.getCityAvailRes();
                yield Prize.takeCityRes();
        ...
        Object.values(Elkaisar.Battel.BattelList).forEach(function (Battel, Index) {
            ...
            Battel.HeroReadyList.forEach(function (Hero, HeroIndex) {
                if (parseInt(Hero.idHero) <= 0)
                    return;
                Elkaisar.DB.Insert(`id_hero = ${Hero.idHero}, x_from  = ${Battel.Battel.x_coord}, y_from = ${Battel.Battel.y_coord},
    ...
    announceGuildWin() {
        var This = this;
        Elkaisar.DB.SelectFrom("guild_member.id_guild, guild_member.id_player, guild.slog_top, guild.slog_cnt, guild.slog_btm, guild.name AS GuildName, player.name AS PlayerName", "guild_member JOIN guild ON guild.id_guild = guild_member.id_guild JOIN player ON player.id_player = guild_member.id_player", "guild_member.id_player = ?", [This.Battel.Battel.id_player], function (PlayerGuild) {
...
LAfterFight.RealHeroCount = 450000;
LAfterFight.SeaCityMinAttackHero = 80;
