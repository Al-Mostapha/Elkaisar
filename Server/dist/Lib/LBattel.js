...
class LBattel {
    static addUnitGarrison(Battel, callBack) {
        Elkaisar.DB.SelectFrom(`world_unit_garrison.id_player, world_unit_garrison.id_hero, world_unit_garrison.x_coord AS x,
                world_unit_garrison.y_coord AS y, hero.id_city, 
                hero.point_b, hero.point_b_plus, hero.point_c, hero.point_c_plus  , 
                hero_medal.medal_den , hero_medal.medal_leo, city.x ,city.y `, `world_unit_garrison JOIN hero ON hero.id_hero = world_unit_garrison.id_hero
                JOIN hero_medal ON hero_medal.id_hero = world_unit_garrison.id_hero
                JOIN city ON city.id_city = hero.id_city`, `world_unit_garrison.x_coord = ? AND world_unit_garrison.y_coord = ? ORDER BY world_unit_garrison.ord ASC`, [Battel.x_coord, Battel.y_coord], function (Res) {
    ...
    static removeHeroFromBattel(idHero, callBack) {
        ...
        Elkaisar.DB.SelectFrom("*", "battel_member", `id_hero = ${idHero}`, [], function (HeroBattel) {
            ...
            for (var idH of Heros) {
                Elkaisar.DB.Insert(`id_hero = ${idH.idHero}, x_from  = ${Battel.Battel.x_coord}, y_from = ${Battel.Battel.y_coord},
    ...
    static addHeroToBattel(Hero, Battel, callBack) {
        ...
        Elkaisar.DB.SelectFrom(`hero.point_a, hero.point_a_plus,hero.point_b, hero.point_b_plus, hero.id_player,
                 hero.point_c, hero.point_c_plus , hero.id_player, hero.avatar, hero.name AS HeroName,
                 hero_medal.medal_den , hero_medal.medal_leo, 
                 hero.id_city, city.x, city.y , city.name AS CityName`, `hero  JOIN hero_medal  ON hero_medal.id_hero = hero.id_hero JOIN city ON hero.id_city = city.id_city`, `hero.id_hero = ?`, [Hero.idHero], function (Res) {
            var CuHero = Res[0];
            if (!CuHero)
                return;
