...
Elkaisar.World.refreshWorldUnit = function (callBack) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const Units = yield Elkaisar.DB.ASelectFrom("x, y, l, t, ut, lo", "world", "1", []);
        var ii;
        for (ii in Units) {
            const Unit = Units[ii];
...
Elkaisar.World.getUnitData = function (callBack) {
    ...
    }, `${Elkaisar.CONST.BASE_URL}/js/json/worldUnitData.json`, function (data) {
...
class LWorld {
    ...
    static battelHeros(Battel, callBack) {
        Elkaisar.DB.SelectFrom(`battel_member.id_hero , battel_member.id_player ,
                hero.point_b, hero.point_b_plus, hero.point_c, hero.point_c_plus  , 
                hero_medal.medal_den , hero_medal.medal_leo , battel_member.ord , 
                hero.id_city  , battel_member.side , city.x, city.y `, `battel_member JOIN  hero ON hero.id_hero = battel_member.id_hero JOIN hero_medal  ON hero_medal.id_hero = battel_member.id_hero JOIN city ON hero.id_city = city.id_city`, `battel_member.id_battel = ? ORDER BY battel_member.ord ASC`, [Battel.id_battel], function (Res) {
            callBack(Res);
        });
    }
