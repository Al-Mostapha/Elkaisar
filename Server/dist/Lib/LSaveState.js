...
class LSaveState {
    static saveCityState(idCity, callBack) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield Elkaisar.DB.AUpdate(`coin = GREATEST( LEAST( coin   + (CAST(coin_in   AS SIGNED) - CAST(coin_out   AS SIGNED))*(${now} - LS)/3600 , GREATEST( coin_cap , coin )  ) , 0), 
    ...
    static foodOutState(idPlayer, idCity, callBack) {
        Elkaisar.DB.SelectFrom("hero_army.*", "hero_army JOIN hero ON hero.id_hero = hero_army.id_hero", "hero.id_city = ?", [idCity], function (Heros) {
            ...
            Elkaisar.DB.Exist("city_colonize", "id_city_colonized = ?", [idCity], function (Found) {
                ...
                Elkaisar.DB.Update(`food_out = LEAST( (army_a*4 + army_b*18 + army_c*36 + army_d*5 + army_e*20 + army_f*150 + ?) * ( 1 - (SELECT  supplying FROM  player_edu WHERE id_player = ?)*3/100 ),  food_in)`, "city", "id_city = ?", [heroFood, idPlayer, idCity], function (Res) {
                    if (callBack)
                        callBack();
                });
            });
    ...
    static getConsoleEffect(idCity, callBack) {
        var consoleEffect = 0;
        Elkaisar.DB.SelectFrom("point_a, point_a_plus, medal_ceasro", "hero JOIN city ON city.console = hero.id_hero JOIN hero_medal ON hero_medal.id_hero = city.console", "city.id_city = ?", [idCity], function (Res) {
            if (!Res)
                return callBack(0);
            if (!Res[0])
                return callBack(0);
    ...
    static getResFromColonize(idCity, callBack) {
        Elkaisar.DB.SelectFrom("city_colonize.id_city_colonized, city.food_in, city.wood_in, city.stone_in, city.metal_in, city.coin_in", "city_colonize JOIN city ON city.id_city = city_colonize.id_city_colonized", "city_colonize.id_city_colonizer = ?", [idCity], function (Cities) {
            var Res = {
                "food": 0,
                "wood": 0,
                "stone": 0,
    ...
    static resInState(idPlayer, idCity, Reso, ColoEff = 0) {
        Elkaisar.DB.SelectFrom("*", "city_jop", "id_city = ?", [idCity], function (Res) {
            ...
            Elkaisar.DB.SelectFrom(LSaveState.ResourceInEffct[Reso]["PlayerState"], "player_stat", "id_player = ?", [idPlayer], function (PlayerState) {
                ...
                Elkaisar.DB.SelectFrom("IFNULL(SUM(world.l),0 ) as lvlsum", "world JOIN city_bar ON world.x = city_bar.x_coord AND  world.y = city_bar.y_coord", `city_bar.id_city = ? AND world.ut BETWEEN  ${LSaveState.ResourceInEffct[Reso]["ResBarr"]}`, [idCity], function (BarrLvl) {
