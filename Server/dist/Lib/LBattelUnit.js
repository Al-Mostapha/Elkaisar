...
class LBattelUnit {
    static getBattelById(idBattel) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield Elkaisar.DB.ASelectFrom("battel.*, player.name AS PlayerName, city.name AS CityName, player.id_guild AS idGuild,player.guild AS GuildName", "battel JOIN city ON city.x = battel.x_city AND city.y = battel.y_city  JOIN player ON battel.id_player = player.id_player", "id_battel = ?", [idBattel]);
        });
    }
    static isAttackable(idPlayer, idHero, Unit) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (Elkaisar.Lib.LWorldUnit.isArenaGuild(Unit["ut"]) || Elkaisar.Lib.LWorldUnit.isStatueWar(Unit["ut"])) {
                return (yield Elkaisar.DB.AExist("guild_member", "id_player = ?", [idPlayer]));
            }
            else if (Elkaisar.Lib.LWorldUnit.isRepelCastle(Unit["ut"]) || Elkaisar.Lib.LWorldUnit.isQueenCity(Unit["ut"])) {
                if ((yield Elkaisar.DB.AExist("guild_member", "id_player = ?", [idPlayer])) == false)
                    return false;
                if ((yield Elkaisar.DB.AExist("battel", "x_coord = ? AND y_coord = ?", [Unit.x, Unit.y])) == true)
                    return false;
            }
    ...
    static takeStartingPrice(idPlayer, Unit) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (iii in UnitData.MakeReq) {
                PlayerAmount = yield Elkaisar.DB.ASelectFrom("*", "player_item", "id_player = ? AND id_item = ?", [idPlayer, UnitData.MakeReq[iii].Item]);
                if (!PlayerAmount[0])
                    return false;
    ...
    static takeJoinPrice(idPlayer, ut) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (iii in UnitData.JoinReq) {
                PlayerAmount = yield Elkaisar.DB.ASelectFrom("*", "player_item", "id_player = ? AND id_item = ?", [idPlayer, UnitData.JoinReq[iii].Item]);
                if (!PlayerAmount[0])
                    return false;
    ...
    static onTheRoleInAttQue(idPlayer, Unit) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!Elkaisar.Lib.LWorldUnit.isRepelCastle(Unit["ut"]))
                return true;
            const GuildPlayer = yield Elkaisar.DB.ASelectFrom("id_player, id_guild, rank", "guild_member", "id_player = ?", [idPlayer]);
            if (!GuildPlayer[0])
                return false;
            if (GuildPlayer[0]["rank"] < Elkaisar.Config.GUILD_R_DEPUTY_2)
                return false;
            const QueueRole = yield Elkaisar.DB.ASelectFrom("*", "world_attack_queue", "x_coord = ? AND y_coord = ? ORDER BY id ASC LIMIT 1", [Unit["x"], Unit["y"]]);
            if (!QueueRole.length)
                return false;
    ...
    static calAttackTime(City, Unit, slowestSpeed) {
        if (Elkaisar.Lib.LWorldUnit.isAsianSquads(Unit.ut) || Elkaisar.Lib.LWorldUnit.isGangStar(Unit.ut)
    ...
    static getBattelById(idBattel) {
        return __awaiter(this, void 0, void 0, function* () {
            return (yield Elkaisar.DB.ASelectFrom("battel.*, player.name AS PlayerName, city.name AS CityName, player.id_guild AS idGuild,player.guild AS GuildName", "battel JOIN city ON city.x = battel.x_city AND city.y = battel.y_city  JOIN player ON battel.id_player = player.id_player", "id_battel = ?", [idBattel]))[0];
        });
    }
    static announceStart(idPlayer, Unit) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!Elkaisar.Lib.LWorldUnit.isQueenCity(Unit["ut"]) && !Elkaisar.Lib.LWorldUnit.isRepelCastle(Unit["ut"]))
                return;
            const Player = yield Elkaisar.DB.ASelectFrom("player.id_player, player.name AS PlayerName, guild.name AS GuildName, guild.slog_top, guild.slog_cnt, guild.slog_btm, player.id_guild", "player LEFT JOIN guild ON guild.id_guild = player.id_guild", "player.id_player = ?", [idPlayer]);
            if (!Player[0])
                return;
    ...
    static join(idPlayer, Battel, Hero, side, Ord = -1) {
        ...
        return Elkaisar.DB.AInsert("id_battel = ? , id_player = ? ,id_hero = ?, id_city = ?,  side = ? , ord = ?", "battel_member", [Battel["id_battel"], idPlayer, Hero["id_hero"], Hero["id_city"], side, Ord]);
    }
    static involvedPlayers(Unit) {
        return __awaiter(this, void 0, void 0, function* () {
            var ArmyCapital = [];
            var City = [];
            var Garrison = yield Elkaisar.DB.ASelectFrom("id_player", "world_unit_garrison", "x_coord = ? AND y_coord = ?", [Unit["x"], Unit["y"]]);
            var Players = {};
            if (Elkaisar.Lib.LWorldUnit.isCity(Unit["ut"]))
                City = yield Elkaisar.DB.ASelectFrom("id_player", "city", "x = ? AND y = ?", [Unit["x"], Unit["y"]]);
            if (Elkaisar.Lib.LWorldUnit.isArmyCapital(Unit["ut"]))
                ArmyCapital = yield Elkaisar.DB.ASelectFrom("id_dominant", "world_unit_rank", "x = ? AND y = ? ORDER BY id_round DESC LIMIT 1", [Unit["x"], Unit["y"]]);
    ...
    static informRelatedPlayers(Battel) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!Elkaisar.Lib.LWorldUnit.isArmyCapital(Unit.ut) && Elkaisar.Lib.LWorldUnit.isCity(Unit.ut))
                return;
            const Attack = yield Elkaisar.DB.ASelectFrom("player.name AS PlayerName, city.name AS CityName, city.x, city.y", "city JOIN player ON player.id_player = city.id_player", "city.x = ? AND city.y = ?", [Battel.x_city, Battel.y_city]);
            if (Elkaisar.Lib.LWorldUnit.isArmyCapital(Unit.ut)) {
                const Defence = yield Elkaisar.DB.ASelectFrom("id_dominant", "world_unit_rank", "x = ? AND y = ? ORDER BY id_round DESC LIMIT 1", [Unit["x"], Unit["y"]]);
                if (!Defence.legnth)
                    return;
    ...
    ;
    static informInvolvedPlayers(Battel) {
        return __awaiter(this, void 0, void 0, function* () {
            const Unit = Elkaisar.World.getUnit(Battel.x_coord, Battel.y_coord);
            const UnitData = Elkaisar.World.WorldUnitData[Unit.ut];
            var InvolvedPlayer = yield Elkaisar.Lib.LBattelUnit.involvedPlayers(Unit);
    ...
    static startBattel(idPlayer, idHero, Hero, Unit, attackTask) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const InsBattel = yield Elkaisar.DB.AInsert("id_hero = ?, time_start = ?, time_end = ?, x_coord = ?, y_coord = ?, id_player = ?, x_city = ? , y_city = ? , task = ?", "battel", [idHero, now, now + attackTime, Unit["x"], Unit["y"], idPlayer, Hero[0]["x"], Hero[0]["y"], attackTask]);
            const Battel = yield Elkaisar.Lib.LBattelUnit.getBattelById(InsBattel.insertId);
